...
float theta_pid_loop(float theta_error,
...
void rover_straight(float dist_reqd) {
  ...
  while (abs(current_dist_error) > max_dist_error) {
    ...
    if (timeoutcounter > 200 || (timeSinceStart - millis())/1000 > 5) {
      break;
    }*/
    if ((abs(current_turn_error) < max_turn_error) ||
        (abs(current_dist_error) < 10)) {
...
void rover_rotate(float theta_reqd) {
  ...
  float current_integral_theta_error = 0;
  ...
  while (abs(current_theta_error) > max_theta_error) {
    ...
    /*if (abs(prev_theta_error - current_theta_error) <= 0.05) {
      timeoutcounter++;
    } else {
      timeoutcounter = 0;
    }
    ...
    */
    ...
    // Serial.println(String(current_theta_error));

    motorrotate(leftmotorcontrol, motor1);
    motorrotate(rightmotorcontrol, motor2);
    // Serial.println("Rotate loop: current_theta_error - " +
    // String(abs(current_theta_error)) + "; max_theta_error: "
